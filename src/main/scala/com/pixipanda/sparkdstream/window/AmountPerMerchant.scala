...
object AmountPerMerchant {
  ...
  def main(args: Array[String]) {
    ...
    val windowedDstream = transactionDstream.window(Seconds(30), Seconds(5))


    val pairDstream  = windowedDstream.map(transaction => (transaction.merchant, transaction.amt))

    val last30SecondsAmtPerMerchant = pairDstream.reduceByKey(_ + _)
